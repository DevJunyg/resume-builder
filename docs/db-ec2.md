...
> ⚠️ **먼저 원본(RDS) 버전을 확인하라.** `pg_dump`는 **덤프 도구·복원 대상의 버전이
> 원본 서버 버전 이상**이어야 한다(구 버전 클라이언트로 신 버전 서버를 덤프하면
> `aborting because of server version mismatch`로 거부). Ubuntu 24.04 apt 기본은 16인데,
> RDS를 18로 만들었다면 EC2도 **18로 맞춰야** 경로 A(pg_dump/restore)가 된다.
> RDS 버전 확인: `psql -h <RDS> -U postgres -d resumebuilder -c "SHOW server_version;"`

### 원본과 같은 메이저 버전 설치 (예: 18, PGDG 저장소)

Ubuntu 기본 저장소에 없는 버전은 PostgreSQL 공식 저장소(PGDG)를 추가해 설치한다.
...
psql --version                 # 18.x 확인
pg_lsclusters                  # 18 main 이 5432 online 인지
```

> RDS와 EC2 버전이 애초에 같다면(둘 다 16 등) PGDG 없이 `sudo apt install -y postgresql postgresql-contrib` 로 충분하다.
> 설치했는데 `pg_lsclusters`가 비어 있으면(클러스터 자동 생성 누락) 직접 만든다:
...
```bash
# 설치된 메이저 버전 확인 (아래는 18 가정 — 다르면 경로의 18을 바꿔라)
ls /etc/postgresql/
...
-- ⚠️ PostgreSQL 15+ 는 일반 유저가 public 스키마에 CREATE 권한이 없다.
--    DB를 appuser 소유로 만들면 appuser가 public 스키마도 다룰 수 있어 가장 깔끔하다.
CREATE DATABASE resumebuilder OWNER appuser;
\c resumebuilder
ALTER SCHEMA public OWNER TO appuser;
\q
```

> **PG15+ public 스키마 함정**: 예전엔 아무나 public에 테이블을 만들 수 있었지만, 15부터
> 막혔다. 그래서 `appuser`로 `pg_restore`(경로 A)나 마이그레이션(경로 B)을 돌리면
> `permission denied for schema public`으로 실패한다. 위처럼 **DB/스키마 소유자를 appuser로**
> 두면 해결된다. (이미 `postgres` 소유로 DB를 만들었다면
> `sudo -u postgres psql -d resumebuilder -c "ALTER SCHEMA public OWNER TO appuser;"` 한 줄로 교정.)
...
전제: **EC2의 pg_dump/서버 버전 ≥ RDS 버전** (§3 참고). RDS가 18이면 EC2도 18이어야 한다.
...
# 2) EC2 로컬 PostgreSQL로 복원
#    --no-owner: 원본 소유자(postgres)가 EC2엔 없으니 무시 → 접속 유저(appuser) 소유로
#    --no-privileges: 원본 GRANT 구문(다른 롤 대상) 건너뜀 → 잡음 감소
PGPASSWORD='appuser_비번' pg_restore \
  -h localhost -U appuser -d resumebuilder --no-owner --no-privileges ~/rds.dump
```

> `-Fc`(custom) 포맷은 `pg_restore`로 선택 복원·병렬 복원이 가능해 실무 표준이다.
> 순수 SQL 텍스트가 필요하면 `-Fp`(plain) + `psql -f`.
>
> `permission denied for schema public`으로 CREATE TABLE이 줄줄이 실패하면 §5의
> **PG15+ public 스키마 함정**이다 — `ALTER SCHEMA public OWNER TO appuser;` 후 재실행.
> `role "rds_superuser" does not exist` 류 **경고**는 무시해도 된다(데이터는 정상 복원).
